...
var g_ms_ie = ((old_ie > -1) || (new_ie > -1)) ? true : false;
var g_chrome = ua.toLowerCase().indexOf('chrome') > -1;
...
function update_editor_size() {
  if (g_active_tab_id == 3) {
    ...
    if (g_use_webdialog) {
      ...
      sketchup.editor_size_changed(data);
    }
  }
}
...
function update_placement(tab_id, bresize) {
  ...
  if (g_use_webdialog) {
    ...
    sketchup.placement_changed(data);
  }
...
function assign_joint_click_event() {
  ...
  $('.joint-label').on('click', function(evt) {
    ...
    if (g_use_webdialog) {
      ...
      sketchup.joint_label_selected(this.id);
    }
  });
}
...
function update_input_events() {
  ...
  $('.numeric-input').on('focusout', function() {
    ...
    if (g_use_webdialog) {
      if (g_ms_ie) {
        ...
        window.setTimeout( function() { callback('numeric_input_changed', data); }, 10);
      }
    }
    else {
      sketchup.numeric_input_changed(data);
    }
  });
  ...
  $('.fixnum-input').on('focusout', function() {
    ...
    if (g_use_webdialog) {
      if (g_ms_ie) {
        ...
        window.setTimeout( function() { callback('fixnum_input_changed', data); }, 10);
      }
    }
    else {
      sketchup.fixnum_input_changed(data);
    }
  });
  ...
  $('.controller-input').on('focusout', function() {
    if (g_use_webdialog) {
      ...
      if (g_ms_ie) {
        ...
        window.setTimeout( function() { callback('controller_input_changed', data); }, 10);
      }
    }
    else {
      var data = [this.id, this.value];
      sketchup.controller_input_changed(data);
    }
  });
  ...
  $('.slider-input').on('focusout', function() {
    ...
    if (g_use_webdialog) {
      if (g_ms_ie) {
        ...
        window.setTimeout( function() { callback('slider_input_changed', data); }, 10);
      }
    }
    else {
      sketchup.slider_input_changed(data);
    }
  });
  ...
  $('.text-input').on('focusout', function() {
    if (g_use_webdialog) {
      ...
      if (g_ms_ie) {
        ...
        window.setTimeout( function() { callback('text_input_changed', data); }, 10);
      }
    }
    else {
      var data = [this.id, this.value];
      sketchup.text_input_changed(data);
    }
  });
  ...
  $('.numeric-input').on('focusin', function() {
    var data = this.id;
    if (g_use_webdialog) {
      callback('numeric_input_focused', data);
    }
    else {
      sketchup.numeric_input_focused(data);
    }
  });
  ...
  $('input[type=checkbox], input[type=radio]').on('change', function() {
    ...
    if (g_use_webdialog) {
      ...
      sketchup.check_input_changed(data);
    }
  });
  ...
  $('.button').on('click', function(evt) {
    var data = this.id;
    if (g_use_webdialog) {
      callback('button_clicked', data);
    }
    else {
      sketchup.button_clicked(data);
    }
    evt.preventDefault();
  });
}
...
$(document).ready( function() {
  ...
  $('.drop-down').on('change', function(evt, params) {
    ...
    if (g_use_webdialog) {
      ...
      sketchup.select_input_changed(data);
    }
  });
  ...
  $('#sound-list').on('change', function() {
    var data = this.value;
    if (g_use_webdialog) {
      callback('sound_select_changed', data);
    }
    else {
      sketchup.sound_select_changed(data);
    }
  });
