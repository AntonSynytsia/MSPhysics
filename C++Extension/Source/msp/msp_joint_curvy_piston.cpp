...
const bool MSP::CurvyPiston::DEFAULT_ROTATION_ENABLED(true);
const int MSP::CurvyPiston::DEFAULT_CONTROLLER_MODE(0);
...
void MSP::CurvyPiston::c_clear_curve_edges(MSP::Joint::JointData* joint_data) {
...
bool MSP::CurvyPiston::c_calc_curve_data_at_position(const MSP::Joint::JointData* joint_data, dFloat position, dMatrix& normal_matrix, dFloat &distance, dFloat &overpass) {
...
bool MSP::CurvyPiston::c_calc_curve_data_at_point(const MSP::Joint::JointData* joint_data, const dVector &location, dMatrix& normal_matrix, dFloat &distance, dFloat &overpass) {
	if (!joint_data->m_connected) return false;
	const CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
bool MSP::CurvyPiston::c_calc_curve_data_at_point2(const MSP::Joint::JointData* joint_data, const dVector& location, dVector& point, dVector& vector, dFloat &distance, unsigned int &edge_index) {
	const CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	unsigned int points_size = (unsigned int)cj_data->m_points.size();
...
bool MSP::CurvyPiston::c_calc_curve_data_at_point3(const MSP::Joint::JointData* joint_data, dFloat last_dist, const dVector &location, dMatrix& normal_matrix, dFloat &distance, dFloat &overpass) {
	if (!joint_data->m_connected) return false;
	const CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
void MSP::CurvyPiston::c_calc_pivot_normal(const dMatrix& normal_matrix1, const dMatrix& normal_matrix2, const dVector& pivot_point, const dVector& location, dMatrix& normal_matrix_out) {
...
void MSP::CurvyPiston::submit_constraints(const NewtonJoint* joint, dFloat timestep, int thread_index) {
	...
	if (cj_data->m_controller_enabled) {
		...
		des_accel = (des_vel - cj_data->m_cur_vel) * inv_timestep;
	}
...
void MSP::CurvyPiston::get_info(const NewtonJoint* const joint, NewtonJointRecord* const info) {
}

void MSP::CurvyPiston::on_destroy(MSP::Joint::JointData* joint_data) {
	delete (reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data));
}

void MSP::CurvyPiston::on_connect(MSP::Joint::JointData* joint_data) {
...
void MSP::CurvyPiston::adjust_pin_matrix_proc(MSP::Joint::JointData* joint_data, dMatrix& pin_matrix) {
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_is_valid(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* address = reinterpret_cast<MSP::Joint::JointData*>(Util::value_to_ull(v_joint));
	return (MSP::Joint::c_is_joint_valid(address) && address->m_jtype == MSP::Joint::CURVY_PISTON) ? Qtrue : Qfalse;
}

VALUE MSP::CurvyPiston::rbf_create(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::NONE);

	joint_data->m_dof = 6;
	joint_data->m_jtype = MSP::Joint::CURVY_PISTON;
	joint_data->m_cj_data = new CurvyPistonData();
...
VALUE MSP::CurvyPiston::rbf_add_point(VALUE self, VALUE v_joint, VALUE v_position) {
...
VALUE MSP::CurvyPiston::rbf_remove_point(VALUE self, VALUE v_joint, VALUE v_point_index) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_points(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	dMatrix pin_matrix;
...
VALUE MSP::CurvyPiston::rbf_get_points_size(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_points.size());
}

VALUE MSP::CurvyPiston::rbf_clear_points(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_point_position(VALUE self, VALUE v_joint, VALUE v_point_index) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	unsigned int point_index = Util::value_to_uint(v_point_index);
	if (point_index < cj_data->m_points.size()) {
...
VALUE MSP::CurvyPiston::rbf_set_point_position(VALUE self, VALUE v_joint, VALUE v_point_index, VALUE v_position) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	unsigned int point_index = Util::value_to_uint(v_point_index);
	if (point_index < cj_data->m_points.size()) {
...
VALUE MSP::CurvyPiston::rbf_get_length(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_curve_len * M_INCH_TO_METER);
}

VALUE MSP::CurvyPiston::rbf_get_cur_position(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_cur_pos * M_INCH_TO_METER);
}

VALUE MSP::CurvyPiston::rbf_get_cur_velocity(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_cur_vel * M_INCH_TO_METER);
}

VALUE MSP::CurvyPiston::rbf_get_cur_acceleration(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_cur_accel * M_INCH_TO_METER);
}

VALUE MSP::CurvyPiston::rbf_get_cur_point(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_cur_vector(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	if (cj_data->m_cur_normal_matrix_set)
...
VALUE MSP::CurvyPiston::rbf_get_cur_normal_matrix(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	if (cj_data->m_cur_normal_matrix_set)
...
VALUE MSP::CurvyPiston::rbf_get_angular_friction(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_rate(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_power(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_reduction_ratio(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_controller(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	if (cj_data->m_controller_enabled) {
...
VALUE MSP::CurvyPiston::rbf_set_controller(VALUE self, VALUE v_joint, VALUE v_controller) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_controller_mode(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_controller_mode);
}

VALUE MSP::CurvyPiston::rbf_set_controller_mode(VALUE self, VALUE v_joint, VALUE v_mode) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_loop_enabled(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_loop);
}

VALUE MSP::CurvyPiston::rbf_enable_loop(VALUE self, VALUE v_joint, VALUE v_state) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_alignment_enabled(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_align);
}

VALUE MSP::CurvyPiston::rbf_enable_alignment(VALUE self, VALUE v_joint, VALUE v_state) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_alignment_power(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_rotation_enabled(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
	return Util::to_value(cj_data->m_rotate);
}

VALUE MSP::CurvyPiston::rbf_enable_rotation(VALUE self, VALUE v_joint, VALUE v_state) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
VALUE MSP::CurvyPiston::rbf_get_normal_martix_at_position(VALUE self, VALUE v_joint, VALUE v_position) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
...
VALUE MSP::CurvyPiston::rbf_get_normal_martix_at_point(VALUE self, VALUE v_joint, VALUE v_point) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
...
VALUE MSP::CurvyPiston::rbf_get_normal_matrices(VALUE self, VALUE v_joint) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::CURVY_PISTON);
	CurvyPistonData* cj_data = reinterpret_cast<CurvyPistonData*>(joint_data->m_cj_data);
...
void MSP::CurvyPiston::init_ruby(VALUE mNewton) {
	VALUE mCurvyPiston = rb_define_module_under(mNewton, "CurvyPiston");

	rb_define_module_function(mCurvyPiston, "is_valid?", VALUEFUNC(MSP::CurvyPiston::rbf_is_valid), 1);
	rb_define_module_function(mCurvyPiston, "create", VALUEFUNC(MSP::CurvyPiston::rbf_create), 1);
	rb_define_module_function(mCurvyPiston, "add_point", VALUEFUNC(MSP::CurvyPiston::rbf_add_point), 2);
	rb_define_module_function(mCurvyPiston, "remove_point", VALUEFUNC(MSP::CurvyPiston::rbf_remove_point), 2);
	rb_define_module_function(mCurvyPiston, "get_points", VALUEFUNC(MSP::CurvyPiston::rbf_get_points), 1);
	rb_define_module_function(mCurvyPiston, "get_points_size", VALUEFUNC(MSP::CurvyPiston::rbf_get_points), 1);
	rb_define_module_function(mCurvyPiston, "clear_points", VALUEFUNC(MSP::CurvyPiston::rbf_clear_points), 2);
	rb_define_module_function(mCurvyPiston, "get_point_position", VALUEFUNC(MSP::CurvyPiston::rbf_get_point_position), 2);
	rb_define_module_function(mCurvyPiston, "set_point_position", VALUEFUNC(MSP::CurvyPiston::rbf_set_point_position), 3);
	rb_define_module_function(mCurvyPiston, "get_length", VALUEFUNC(MSP::CurvyPiston::rbf_get_length), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_position", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_position), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_velocity", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_velocity), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_acceleration", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_acceleration), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_point", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_point), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_vector", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_vector), 1);
	rb_define_module_function(mCurvyPiston, "get_cur_normal_matrix", VALUEFUNC(MSP::CurvyPiston::rbf_get_cur_normal_matrix), 1);
	rb_define_module_function(mCurvyPiston, "get_angular_friction", VALUEFUNC(MSP::CurvyPiston::rbf_get_angular_friction), 1);
	rb_define_module_function(mCurvyPiston, "set_angular_friction", VALUEFUNC(MSP::CurvyPiston::rbf_set_angular_friction), 2);
	rb_define_module_function(mCurvyPiston, "get_rate", VALUEFUNC(MSP::CurvyPiston::rbf_get_rate), 1);
	rb_define_module_function(mCurvyPiston, "set_rate", VALUEFUNC(MSP::CurvyPiston::rbf_set_rate), 2);
	rb_define_module_function(mCurvyPiston, "get_power", VALUEFUNC(MSP::CurvyPiston::rbf_get_power), 1);
	rb_define_module_function(mCurvyPiston, "set_power", VALUEFUNC(MSP::CurvyPiston::rbf_set_power), 2);
	rb_define_module_function(mCurvyPiston, "get_reduction_ratio", VALUEFUNC(MSP::CurvyPiston::rbf_get_reduction_ratio), 1);
	rb_define_module_function(mCurvyPiston, "set_reduction_ratio", VALUEFUNC(MSP::CurvyPiston::rbf_set_reduction_ratio), 2);
	rb_define_module_function(mCurvyPiston, "get_controller", VALUEFUNC(MSP::CurvyPiston::rbf_get_controller), 1);
	rb_define_module_function(mCurvyPiston, "set_controller", VALUEFUNC(MSP::CurvyPiston::rbf_set_controller), 2);
	rb_define_module_function(mCurvyPiston, "get_controller_mode", VALUEFUNC(MSP::CurvyPiston::rbf_get_controller_mode), 1);
	rb_define_module_function(mCurvyPiston, "set_controller_mode", VALUEFUNC(MSP::CurvyPiston::rbf_set_controller_mode), 2);
	rb_define_module_function(mCurvyPiston, "loop_enabled?", VALUEFUNC(MSP::CurvyPiston::rbf_loop_enabled), 1);
	rb_define_module_function(mCurvyPiston, "enable_loop", VALUEFUNC(MSP::CurvyPiston::rbf_enable_loop), 2);
	rb_define_module_function(mCurvyPiston, "alignment_enabled?", VALUEFUNC(MSP::CurvyPiston::rbf_alignment_enabled), 1);
	rb_define_module_function(mCurvyPiston, "enable_alignment", VALUEFUNC(MSP::CurvyPiston::rbf_enable_alignment), 2);
	rb_define_module_function(mCurvyPiston, "get_alignment_power", VALUEFUNC(MSP::CurvyPiston::rbf_get_alignment_power), 1);
	rb_define_module_function(mCurvyPiston, "set_alignment_power", VALUEFUNC(MSP::CurvyPiston::rbf_set_alignment_power), 2);
	rb_define_module_function(mCurvyPiston, "rotation_enabled?", VALUEFUNC(MSP::CurvyPiston::rbf_rotation_enabled), 1);
	rb_define_module_function(mCurvyPiston, "enable_rotation", VALUEFUNC(MSP::CurvyPiston::rbf_enable_rotation), 2);
	rb_define_module_function(mCurvyPiston, "get_normal_martix_at_position", VALUEFUNC(MSP::CurvyPiston::rbf_get_normal_martix_at_position), 2);
	rb_define_module_function(mCurvyPiston, "get_normal_martix_at_point", VALUEFUNC(MSP::CurvyPiston::rbf_get_normal_martix_at_point), 2);
	rb_define_module_function(mCurvyPiston, "get_normal_matrices", VALUEFUNC(MSP::CurvyPiston::rbf_get_normal_matrices), 1);
}
