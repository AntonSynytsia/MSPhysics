...
const dFloat MSP::Spring::DEFAULT_MIN(-10.0f);
const dFloat MSP::Spring::DEFAULT_MAX(10.0f);
...
const dFloat MSP::Spring::DEFAULT_ACCEL(40.0f);
const dFloat MSP::Spring::DEFAULT_DAMP(0.1f);
const dFloat MSP::Spring::DEFAULT_STRENGTH(0.8f);
const dFloat MSP::Spring::DEFAULT_SPRING_CONSTANT(40.0f);
const dFloat MSP::Spring::DEFAULT_SPRING_DRAG(1.0f);
const dFloat MSP::Spring::DEFAULT_START_POSITION(0.0f);
const dFloat MSP::Spring::DEFAULT_CONTROLLER(1.0f);
...
VALUE MSP::Spring::rbf_set_spring_constant(VALUE self, VALUE v_joint, VALUE v_spring_constant) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::SPRING);
	SpringData* cj_data = reinterpret_cast<SpringData*>(joint_data->m_cj_data);
	cj_data->m_spring_constant = Util::max_float(Util::value_to_dFloat(v_spring_constant) * M_METER_TO_INCH, 0.0f);
	return Qnil;
}
...
VALUE MSP::Spring::rbf_set_spring_drag(VALUE self, VALUE v_joint, VALUE v_spring_drag) {
	MSP::Joint::JointData* joint_data = MSP::Joint::c_value_to_joint2(v_joint, MSP::Joint::SPRING);
	SpringData* cj_data = reinterpret_cast<SpringData*>(joint_data->m_cj_data);
	cj_data->m_spring_drag = Util::max_float(Util::value_to_dFloat(v_spring_drag) * M_METER_TO_INCH, 0.0f);
	return Qnil;
}
